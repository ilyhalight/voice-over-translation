...
- Реализована `WEB_ABR` - новая гибридная стратегия загрузки аудиодорожки с ютуба.
  ...
  - использует несколько различных вариантов клиента (`web_embedded` -> `tv_downgraded` -> `web` -> `web_creator` (требует вход в YouTube аккаунт, но позволяет переводить age-restricted видео)) (порт из yt-dlp), включая `web` поддерживающий age-restricted видео
  - почанковая загрузка
  - приоритеты itag из `WEB_ABR`

  Минусы:
  - сильно повышает размер бандла (+30%)
