# 1.12.0 - The Next Update (WIP)

Это обновление стало одним из самых масштабных за последнее время. Основное внимание уделено полной миграции интерфейса на SolidJS, его заметному редизайну и переработке множества UI-элементов. Вместе с этим улучшена работа субтитров, исправлен ряд накопившихся ошибок и упрощено дальнейшее развитие интерфейса.

## Важное изменение в системе обновлений

> [!NOTE]
> Для перехода на новую систему обновлений достаточно обновить расширение до этой версии или любой более новой.

Начиная с этой версии мы постепенно отказываемся от хранения готовых сборок расширения непосредственно в репозитории ([dist](https://github.com/ilyhalight/voice-over-translation/tree/master/dist) / [dist-ext](https://github.com/ilyhalight/voice-over-translation/tree/master/dist-ext)).

Основным местом публикации новых версий станет раздел [Releases](https://github.com/ilyhalight/voice-over-translation/releases). Как и раньше, расширение также будет доступно через официальные магазины [Chrome Web Store](https://chromewebstore.google.com/detail/dnioaagdjgpcokckfpokpndoblenmfcg) и [Firefox Add-ons](https://addons.mozilla.org/ru/firefox/addon/voice-over-translation/).

На переходном этапе файлы в [dist](https://github.com/ilyhalight/voice-over-translation/tree/master/dist) будут продолжать обновляться для обеспечения совместимости с существующими механизмами установки и обновления. В дальнейшем их публикация будет полностью прекращена.

## Основные изменения

- Проведен редизайн некоторых элементов интерфейса расширения
  - Переработан дизайн блока настроек "Мой аккаунт"
  - Переработан дизайн блока "О расширении"
  - Переработан дизайн слайдеров
  - Переработан дизайн Select элементов
  - `Checkbox` были заменены на `Switch` элементы
  - Добавлены описания для некоторых настроек
  - Добавлено отображение состояния "не найдено" для результатов поиска в меню выбора
  - Добавлено отображение анимации поиска при поиске через внешние апи в меню выбора (например, при поиске шрифтов для субтитров)
  - Добавлено визуальное выделение при наведении на иконку открытия меню выбора озвучки
  - Добавлены тултипы для кнопок "Картинка в картинке" и "Меню" в кнопке перевода
  - Добавлена возможность навигации по виджету субтитров с помощью клавиатуры:
    - поддержка tab для перехода к следующему слову;
    - пробел или enter для включения/выключения подсказки слова;
    - esc для выключения подсказки слова;
  - Заменены иконки "Стандартных" и "Живых" голосов на более приятные
  - Переработана иконка прогресса загрузки для аудио. Теперь, она стала более плавной, а также обзавелась визуальным выделением незаполненной части прогресса
  - Улучшено визуальное выделение активного слова субтитров в некоторых случаях
- Весь UI расширения переведен на SolidJS
- Слайдер "Громкость при уменьшении" теперь отключается при включенном режиме "Адаптивная громкость"
- Оптимизирован предпросмотр позиции "Кнопки перевода". Теперь, он использует облегченную версию элемента, а не создает полный клон
- Меню выбора "Не переводить с выбранных языков" теперь может содержать 0 выбранных языков (ранее было минимум 1)

  При выборе 0 языков, опция будет считаться отключенной и перевод будет выполняться с любого языка. Если у вас ранее была отключена функция "Не переводить с выбранных языков", то список языков будет автоматически очищен

- Переработан механизм авторизации с помощью Яндекса. Теперь, мы делаем только прямые запросы к API Яндекса, без использования дополнительного сервиса-прослойки (#1795)
- Добавлено настоящее обновление отображаемых данных аккаунта при нажатии на кнопку "обновить" в настройках расширения. Ранее, обновлялись только данные из локального хранилища из-за чего могли отображаться устаревшие данные или неотображаться вовсе (#1769)
- Добавлена автопауза при подготовке перевода (#1741)
- Добавлена отдельная настройка силы приглушения оригинального звука во время перевода с адаптивной громкостью
- Включено изменение положения кнопки перевода для плееров любых размеров. Ранее, кнопка перевода могла быть перемещена в другую часть только в плеерах шириной более 600 пикселей, что могло доставлять дискомфорт на сайтах с маленькими плеерами (например, X)
- Улучшен расчёт максимальной высоты меню для маленьких видео
- Определение страны больше не задерживает появление интерфейса. Перевод и субтитры при необходимости дожидаются определения страны для получения автоматических настроек прокси
- Изменено отображение ошибки при загрузке аватарки пользователя. Теперь, если аватарку не удалось загрузить будет показываться серый круг
- Исправлена ошибка из-за которой при назначении нового хоткея с комбинацией клавиш, совпадающей с частью уже существующего хоткея, срабатывало действие существующего хоткея
- Исправлена ошибка из-за которой при очистке хоткея с помощью "Esc" закрывался диалог настроек
- Исправлена ошибка из-за которой на прямых ссылках на `.mp4` / `.webm` файлы, кнопка перевода появлялась только при наведении на видео
- Исправлена ошибка из-за которой Debug логи попадали в релизную сборку расширения
- Исправлена ошибка из-за которой на некоторых сайтах взаимодействие с текстовыми полями в настройках расширения не блокировало встроенные горячие клавиши веб-сайта (например, паузу видео на YouTube) (#1819)
- Исправлено отсутствие субтитров от сайта при сбое загрузки субтитров от Яндекса
- Исправлено автоматическое скрытие меню после завершения загрузки файла перевода или субтитров
- Исправлена инициализация расширения для Chrome на некоторых веб-сайтах (#1819)
- Субтитры теперь точнее появляются и исчезают в нужный момент
- Улучшено отображение длинных и одновременно появляющихся субтитров
- Исправлена запаздывающая подсветка последних слов в субтитрах YouTube
- Исправлены редкие проблемы со шрифтом субтитров
- Удалена поддержка миграции настроек из версий расширения ниже 1.10.0
- Другие небольшие технические исправления

# 1.11.16

- Исправлена ошибка из-за которой повторное нажатие на кнопку перевода не сбрасывало состояние кнопки перевода, если в ней отображалась ошибка

# 1.11.15

- [эксп.] При загрузке через Web ABR сначала пытаемся загрузить аудио с использованием анонимного доступа, а при необходимости с авторизованной сессией (от @Acobat12)

# 1.11.14

- Улучшена работа стратегии `WEB_ABR` (#1846):
  - Ускорена загрузка аудио с YouTube
  - Снижен расход трафика за счёт выбора аудиофайла меньшего размера
  - Улучшен выбор языка аудиодорожки для видео с несколькими аудиодорожками
  - Добавлен прогрессивный таймаут для отправки аудио при медленном интернете
- Добавлен повтор перевода нажатием на кнопку «Повторить перевод» (#1846)
- Добавлена поддержка сайта JOI Database (`https://www.the-joi-database.com/`) (спасибо @DeMoN288 в [vot.js#132](https://github.com/FOSWLY/vot.js/issues/132))
- Из списка выбора языков удалены "Литовский" и "Латышский" языки (больше не поддерживаются API перевода)

# 1.11.13

> [!CAUTION]
> Если вы используете кастомный адрес прокси-сервера (он же [vot-worker](https://github.com/FOSWLY/vot-worker)), пожалуйста, убедитесь, что он обновлен до новой версии - 1.1.0 ([подробнее](https://github.com/FOSWLY/vot-worker/blob/main/changelog.md#110)). Из-за изменения логики запросов, расширение не будет работать со старыми версиями [vot-worker](https://github.com/FOSWLY/vot-worker)!

- [!] Изменён внутренний провайдер взаимодействия с [vot-worker](https://github.com/FOSWLY/vot-worker).

  Теперь, все запросы к прокси-серверу происходят без дополнительной сериализации в JSON, что позволило уменьшить потребление трафика до 4х раз! Также, благодаря этому, удалось уменьшить количество ошибок вида "Audio downloader failed while uploading chunk" происходящих из-за слишком большого тела запроса

- Улучшена работа стратегии `WEB_ABR`
  - для мультиязычных видео выбирается аудиодорожка на языке оригинала, либо если ее не удается найти, то выбирается дорожка "по умолчанию"
  - предотвращена одновременная загрузка одного и того же видео в некоторых случаях
  - исправлена отправка пустого финального аудиочанка, из-за которой перевод мог не переходить к следующему этапу (спасибо @Acobat12)
  - улучшен выбор аудиоформата и обновление истёкших URL
- Исправлена работа Netacad (#1837)

# 1.11.12

- Реализована `WEB_ABR` - новая гибридная стратегия загрузки аудиодорожки с ютуба.
  - Она работает в разы быстрее чем старая `WEB_MSE_PROXY` (старая стратегия теперь используется в качестве fallback)
  - Работает в загрузчиках юзерскриптов в отличие от `WEB_MSE_PROXY`
  - При наличии авторизации на YouTube, поддерживает загрузку видео, которые имеют "Ограничение по возрасту" (age-restricted) и были недоступны со старой стратегией

  <details>
  <summary>Подробнее о реализации</summary>

  По своей реализации это гибрид стратегий `WEB_ABR` и `WEB_SABR` используемых Яндексом с актуальными методами из yt-dlp.

  Плюсы:
  - вручную собирает сессию Innertube и минтит poToken, а не надеется получить все готовые данные из плеера
  - использует несколько различных вариантов клиента (`web_embedded` -> `tv_downgraded` -> `web` -> `web_creator` (требует вход в YouTube аккаунт, но позволяет переводить age-restricted видео)) (порт из yt-dlp)
  - почанковая загрузка
  - приоритеты itag из оригинального `WEB_ABR`

  Минусы:
  - повышает размер бандла на ~25%

  </details>

- Переработана внутренняя архитектура загрузки аудио
- Улучшена обработка ошибок и отмены загрузки аудио

# 1.11.11

- Расширена поддержка ссылок Coursera. Добавлена поддержка Labs и некоторых других типов ссылок (#1812, [vot.js#130](https://github.com/FOSWLY/vot.js/pull/130))
- Исправлена работа стратегии mseProxy с длинными видео. Теперь, загрузка аудио не прерывается по жестко заданному короткому таймауту
- Увеличено максимальное время ожидания загрузки плеера до 30 секунд
- Актуализированы имитируемые данные браузера в запросах к апи перевода видео
- Добавлено больше дебаг логов для mseProxy

# 1.11.10

- Исправлен баг из-за которого происходил двойной запрос загрузки аудио дорожки видеоролика на YouTube (#1822)

# 1.11.9

- Исправлена загрузка аудио дорожки видеоролика на YouTube, что ранее приводило к ошибкам вида "Возникла ошибка при переводе, попробуйте позже" (#1820)

# 1.11.8

- Добавлена подсветка кнопки субтитров при включении (#1782)
- Добавлена поддержка Wistia Embed (связано с #1329)
- Исправлена активация скрипта для прямых ссылок на `.mp4` / `.webm` файлы, которые имеют `.` в названии (#1384)
- Исправлена работа расширения в старых браузерах (chromium<=110, firefox<=115)
- Исправлена работа на сайте TelegramK (`https://web.telegram.org/k`)
- Исправлена работа Vimeo Embed в случае, если ссылка содержит параметр `app_id`
- Удалена поддержка incestflix

# 1.11.7

> [!NOTE]
> В этой версии был изменен ID дополнений, если вы используете собранное расширение для браузера, вам прийдется самостоятельно переустановить расширение для обновления на новую версию

- Выбирать озвучку стало проще благодаря отображению активного типа голоса и понятным названиям обычных и «Живых» голосов
- «Живые» голоса теперь доступны для перевода на русский с любого распознанного языка
- Повторный вход после истечения авторизации стал понятнее и требует меньше лишних действий
- Кнопку перевода теперь удобнее размещать по бокам плеера с предварительным просмотром позиции, учетом границ экрана и сохранением выбора
- Перевод запускается надежнее при медленном соединении благодаря отображению времени ожидания и автоматическим повторным загрузкам, а запоздавшие ответы больше не сбивают текущий перевод
- Субтитры стали чище и легче читаются за счет улучшенных переносов строк, пунктуации, подсветки слов и отображения встроенного форматирования
- Виджет субтитров теперь можно свободнее размещать, а подсказки с переводом слов стали аккуратнее и удобнее для чтения
- Громкость и отключение звука теперь синхронизируются корректнее, не изменяя сохраненную громкость YouTube
- Перевод на YouTube больше не отключается из-за изменения таймкода в ссылке (#1736)
- Перевод в Firefox больше не отключается после большой перемотки, если плеер заменяет видеоэлемент (#1752)
- Смена качества и повторяющиеся события плеера больше не должны приводить к лишнему сбросу перевода
- Добавлена поддержка `hot.noodlemagazine.com`
- Исправлено центрирование текста в меню выбора голоса на Яндекс Диске
- Повышена стабильность расширения в Chrome и Firefox, включая запуск, работу во встроенных плеерах и системные уведомления
- Улучшена совместимость userscript с менеджерами пользовательских скриптов
- В версиях для Chrome и Firefox теперь корректно отображается установленная версия расширения

# 1.11.6

- Восстановлена работа кнопки в Google Drive (#912)
- Исправлена работа SAP Learning (#1683)
- Исправлена работа Pornhub (#1376)
- Исправлена работа плеера Kodik (#1611)
- Добавлено меню выбора типа озвучки
- Переработан механизм всплывающих подсказок (Tooltip) и логика их позиционирования относительно границ окна
- Упрощена логика валидации аудио: удалены излишние запросы для ускорения старта воспроизведения перевода
- Улучшена логика кэширования: разделено хранение субтитров и переводов в памяти, а также добавлена опция использования устаревшего кэша при сетевых сбоях API
- Добавлены полифиллы для работы субтитров в более старых браузерах.
- Улучшена работа Яндекс Диска: добавлена поддержка пагинации для корректной загрузки видео из вложенных папок
- Проведена небольшая оптимизация кода

# 1.11.5

- Переписан механизм монтирования UI.
- Упрощена логика загрузки аудио.
...
- Добавлена настройка "Субтитры при открытии" (автоматическое включение субтитров при загрузке видео)
- Добавлена настройка "Адаптивная громкость": приглушение исходного видео теперь учитывает реальное наличие речи в переводе
- Добавлена настройка "Умное расположение субтитров" с автоматическим подбором размера и ширины строк под размер плеера
- Проведен редизайн меню и настроек: разделы перевода, субтитров, горячих клавиш, прокси и внешнего вида теперь оформлены как отдельные секции-аккордеоны
- Переработаны горячие клавиши: добавлен отдельный хоткей для субтитров и улучшена запись/отображение комбинаций клавиш
- Настройка положения кнопки перевода обновлена и вынесена в раздел "Внешний вид"
- Улучшены перенос, выравнивание и позиционирование субтитров в плеере
- Улучшена обработка ошибок при загрузке аудио для перевода
- Переведена сборка проекта на Vite и добавлена сборка браузерного расширения (Chrome/Firefox)
