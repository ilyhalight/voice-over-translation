				...
				try {
					...
					class AccountButton {
						...
						constructor({ loggedIn: d = !1, username: f = "unnamed", avatarId: p = "0/0-0" } = {}) {
							...
							this.container = m.container, this.accountWrapper = m.accountWrapper, this.buttons = m.buttons, this.usernameEl = m.usernameEl, this.avatarEl = m.avatarEl, this.avatarImg = m.avatarImg, this.actionButton = m.actionButton, this.refreshButton = m.refreshButton;
						}
						createElements() {
							...
							let C = h.A.createIconButton(_.M9);
							return C.addEventListener("click", () => {
								this.onRefresh.dispatch();
							}), b.append(x, C), d.append(f, b), {
								container: d,
								accountWrapper: f,
								buttons: b,
								usernameEl: g,
						...
						addEventListener(d, f) {
							return d === "click" ? this.onClick.addListener(f) : d === "refresh" && this.onRefresh.addListener(f), this;
						}
						removeEventListener(d, f) {
							return d === "click" ? this.onClick.removeListener(f) : d === "refresh" && this.onRefresh.removeListener(f), this;
						}
						get buttonText() {
						...
						set loggedIn(d) {
							this._loggedIn = d, this.accountWrapper.hidden = !this._loggedIn, this.actionButton.textContent = this.buttonText;
						}
						get avatarId() {
    ...
    <path fill="currentColor" d="M20 9a1 1 0 0 1 1 1v1a8 8 0 0 1-8 8H9.414l.793.793a1 1 0 0 1-1.414 1.414l-2.496-2.496a1 1 0 0 1-.287-.567L6 17.991a1 1 0 0 1 .237-.638l.056-.06l2.5-2.5a1 1 0 0 1 1.414 1.414L9.414 17H13a6 6 0 0 0 6-6v-1a1 1 0 0 1 1-1m-4.793-6.207l2.5 2.5a1 1 0 0 1 0 1.414l-2.5 2.5a1 1 0 1 1-1.414-1.414L14.586 7H11a6 6 0 0 0-6 6v1a1 1 0 1 1-2 0v-1a8 8 0 0 1 8-8h3.586l-.793-.793a1 1 0 0 1 1.414-1.414"/>
		...
		"./src/ui/manager.ts": (d, f, p) => {
			...
			p.a(d, async (d, m) => {
				try {
					p.d(f, { o: () => UIManager });
					var h = p("./node_modules/browser-id3-writer/dist/browser-id3-writer.mjs"), g = p("./node_modules/@vot.js/shared/dist/utils/subs.js"), _ = p("./src/ui.js"), v = p("./src/ui/views/overlay.ts"), b = p("./src/ui/views/settings.ts"), x = p("./src/utils/utils.js"), C = p("./src/localization/localizationProvider.ts"), w = p("./src/config/config.js"), T = p("./src/ui/components/votButton.ts"), E = p("./src/utils/storage.ts"), D = p("./src/utils/debug.ts"), O = p("./src/utils/VOTLocalizedError.js"), A = d([
						_,
						v,
						b,
						x,
					...
					[_, v, b, x, C, T, E, O] = A.then ? (await A)() : A;
					class UIManager {
						root;
						portalContainer;
						tooltipLayoutRoot;
						...
						initUI() {
							...
							return this.initialized = !0, this.votGlobalPortal = _.A.createPortal(), document.documentElement.appendChild(this.votGlobalPortal), this.votOverlayView = new v.i({
								root: this.root,
							...
							}), this.votOverlayView.initUI(), this.votSettingsView = new b.r({
								globalPortal: this.votGlobalPortal,
								data: this.data,
								videoHandler: this.videoHandler
						...
						initUIEvents() {
							...
							this.votOverlayView.initUIEvents(), this.votOverlayView.addEventListener("click:translate", async () => {
								...
								this.videoHandler?.subtitlesWidget.releaseTooltip(), this.votSettingsView.open(), await (0, x.Eh)();
								...
								if (!(!this.votOverlayView.isInitialized() || !this.videoHandler?.downloadTranslationUrl || !this.videoHandler.videoData)) {
									try {
										...
										let d = await (0, x.G3)(this.videoHandler.downloadTranslationUrl, { timeout: 0 });
										if (!d.ok) throw Error(`HTTP ${d.status}`);
										let f = +d.headers.get("Content-Length"), p = d.body.getReader(), m = new Uint8Array(f);
										this.votOverlayView.downloadTranslationButton.progress = 0;
										let g = 0;
										for (;;) {
											let { done: d, value: h } = await p.read();
											if (d) break;
											m.set(h, g), g += h.length, this.votOverlayView.downloadTranslationButton.progress = Math.round(g / f * 100);
										}
										let _ = (0, x.Le)(this.videoHandler.videoData.downloadTitle), v = new h.Q(m.buffer);
										v.setFrame("TIT2", _), v.addTag(), (0, x.WN)(v.getBlob(), `${_}.mp3`);
									} catch (d) {
										console.error("[VOT] Download failed:", d), this.transformBtn("error", C.j.get("downloadFailed"));
									}
									this.votOverlayView.downloadTranslationButton.progress = 0;
								}
								...
								let d = this.data.subtitlesDownloadFormat ?? "json", f = (0, g.vk)(this.videoHandler.yandexSubtitles, d), p = new Blob([d === "json" ? JSON.stringify(f) : f], { type: "text/plain" }), m = this.data.downloadWithName ? (0, x.Le)(this.videoHandler.videoData.downloadTitle) : `subtitles_${this.videoHandler.videoData.videoId}`;
								(0, x.WN)(p, `${m}.${d}`);
								...
								this.votOverlayView.translationVolumeSlider.max = this.data.audioBooster ? w.T8 : 100, this.votOverlayView.translationVolumeSlider.value = (0, x.qE)(d, 0, 100);
								...
								this.votOverlayView.updateButtonLayout(f, T.A.calcDirection(f));
								...
								window.open(`${w.Ek}/issues/new?${d}`, "_blank")?.focus();
							}).addEventListener("click:resetSettings", async () => {
								let d = await E.d.list();
								await Promise.all(d.map(async (d) => await E.d.delete(d))), await E.d.set("compatVersion", w.r4), window.location.reload();
							});
						}
						async reloadMenu() {
						...
						async handleTranslationBtnClick() {
							...
							if (D.A.log("[handleTranslationBtnClick] click translationBtn"), this.videoHandler.hasActiveSource()) return D.A.log("[handleTranslationBtnClick] video has active source"), this.videoHandler.stopTranslation(), this;
							if (this.votOverlayView.votButton.status !== "none" || this.votOverlayView.votButton.loading) return D.A.log("[handleTranslationBtnClick] translationBtn isn't in none state"), this.videoHandler.actionsAbortController.abort(), this.videoHandler.stopTranslation(), this;
							try {
								if (D.A.log("[handleTranslationBtnClick] trying execute translation"), !this.videoHandler.videoData?.videoId) throw new O.n("VOTNoVideoIDFound");
								(this.videoHandler.site.host === "vk" && this.videoHandler.site.additionalData === "clips" || this.videoHandler.site.host === "douyin") && (this.videoHandler.videoData = await this.videoHandler.getVideoData()), D.A.log("[handleTranslationBtnClick] Run translateFunc", this.videoHandler.videoData.videoId), await this.videoHandler.translateFunc(this.videoHandler.videoData.videoId, this.videoHandler.videoData.isStream, this.videoHandler.videoData.detectedLanguage, this.videoHandler.videoData.responseLanguage, this.videoHandler.videoData.translationHelp);
							} catch (d) {
						...
						isLoadingText(d) {
							return typeof d == "string" && (d.includes(C.j.get("translationTake")) || d.includes(C.j.get("TranslationDelayed")));
						}
						transformBtn(d, f) {
							if (!this.votOverlayView?.isInitialized()) throw Error("[VOT] OverlayView isn't initialized");
				...
				try {
					...
					class SettingsView {
						...
						initUI() {
							...
							}), this.sendNotifyOnCompleteCheckbox = new w.A({
        ...
        ${v})`, x = _.A.createInformation(`${K.j.get("VOTLocaleHash")}:`, b), w = _.A.createOutlinedButton(K.j.get("VOTUpdateLocaleFiles"));
						...
						releaseUI(d = !1) {
							if (!this.isInitialized()) throw Error("[VOT] SettingsView isn't initialized");
							return this.dialog.remove(), this.accountButtonRefreshTooltip?.release(), this.audioBoosterTooltip?.release(), this.useAudioDownloadCheckboxTooltip?.release(), this.useNewAudioPlayerTooltip?.release(), this.onlyBypassMediaCSPTooltip?.release(), this.translationTextServiceTooltip?.release(), this.proxyTranslationStatusSelectTooltip?.release(), this.initialized = d, this;
						}
						releaseUIEvents(d = !1) {
							if (!this.isInitialized()) throw Error("[VOT] SettingsView isn't initialized");
		...
		"./src/utils/utils.js": (d, f, p) => {
			...
			p.a(d, async (d, m) => {
				try {
					...
					var h = p("./node_modules/bowser/es5.js"), g = p("./node_modules/@vot.js/shared/dist/data/consts.js"), _ = p("./src/config/config.js"), v = p("./src/localization/localizationProvider.ts"), b = p("./src/utils/debug.ts"), x = d([v]);
					v = (x.then ? (await x)() : x)[0];
					let C = navigator.language || navigator.userLanguage, w = .66, T = /(?:https?|www|\bhttp\s+)[^\s/]*?(?:\.\s*[a-z]{2,}|\/)\S*|#[^\s#]+|auto-generated\s+by\s+youtube|provided\s+to\s+youtube\s+by|released\s+on|paypal?|0x[\da-f]{40}|[13][1-9a-z]{25,34}|4[\dab][1-9a-z]{93}|t[1-9a-z]{33}/gi, E = [
						"uk",
						"be",
						"bg",
						"mk",
					...
					function secsToStrTime(d) {
						...
						return m >= w && (f += 1, p = 0), f >= 60 ? v.j.get("translationTakeMoreThanHour") : f === 1 || f === 0 && p > 0 ? v.j.get("translationTakeAboutMinute") : f !== 11 && f % 10 == 1 ? v.j.get("translationTakeApproximatelyMinute2").replace("{0}", f) : ![
							...
							3,
							4
						].includes(f % 10) ? v.j.get("translationTakeApproximatelyMinute").replace("{0}", f) : v.j.get("translationTakeApproximatelyMinutes").replace("{0}", f);
					}
					...
					function cleanText(d, f) {
						return (d + " " + (f || "")).replace(T, "").replace(/[^\p{L}]+/gu, " ").substring(0, 450).trim();
					}
					...
					async function GM_fetch(d, f = {}) {
						...
						try {
							...
							return b.A.log("GM_fetch preventing CORS by GM_xmlhttpRequest", f.message), new Promise((f, h) => {
								GM_xmlhttpRequest({
									method: m.method || "GET",
									url: d,
									responseType: "blob",
