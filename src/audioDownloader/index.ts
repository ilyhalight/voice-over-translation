    ...
    try {
      ...
      for (const attemptedStrategy of attempts) {
        try {
          ...
          if (attemptedStrategy === WEB_ABR_STRATEGY &&
              error instanceof Error &&
              error.message.includes("YOUTUBE_SIGN_IN_SUGGESTED")) {
            signInSuggested = true;
            // Do not start MSE or send an empty fallback audio payload: the
            // user must sign in and explicitly retry this video.
      ...
      this.onDownloadAudioError.dispatch(translationId, videoId, signInSuggested);
    } finally {
