...
export class VOTTranslationHandler {
  ...
  private readonly onDownloadedAudio = async (
    ...
    if (!audioData || audioData.byteLength === 0) {
      debug.error("[VOT][AudioUpload] refusing to upload empty full audio", {
        videoId,
        fileId,
      });
  ...
  private readonly onDownloadedPartialAudio = async (
    ...
    const videoUrl = this.getCanonicalUrl(videoId);
    if (!audioData || audioData.byteLength === 0) {
      debug.error("[VOT][AudioUpload] refusing to upload empty audio chunk", {
        videoId,
        fileId,
        index,
        amount,
  ...
  private readonly onDownloadAudioError = async (
    ...
    if (signInSuggested || !shouldUseFallback) {
      this.finishDownloadFailure(
        new VOTLocalizedError(
          signInSuggested ? "VOTYouTubeSignInSuggested" : "VOTFailedDownloadAudio",
        ),
        runId,
      );
      return;
    ...
    try {
      if (this.requestedFailAudio.has(videoUrl)) {
        ...
        // Never send a synthetic zero-byte audio payload. It can advance the
        // translation into a failed server-side state that cannot be retried
